...
-(void)outbound:(NSString *)Url{
    NSDictionary *Par =@{@"Markingcode":Url,@"PNumber":@"1"};
    
//    NSString *A=[NSString stringWithFormat:@"http://td.xggserve.com/inventorysystem/Outboundgoods/index.php"];
//    NSString *str1 = [A stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
    [[ZSNetTool sharedInstance]GET:@"http://td.xggserve.com/outbound"
                           ...
                           success:^(id responseObject)
     {
         if ([[responseObject objectForKey:@"isok"]intValue]==1) {
             [MBProgressHUD showSuccess:@"出库成功" toView:self.view];
